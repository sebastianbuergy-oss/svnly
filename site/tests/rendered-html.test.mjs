...
test("device installer identifies Codemagic build 13 and its commit", async () => {
  const response = await render("/install/device");
  assert.equal(response.status, 200);
  const html = await response.text();
  assert.match(html, /Codemagic build 13/);
  assert.match(html, /commit e6adf59/);
  assert.match(html, /itms-services:\/\/\?action=download-manifest/);
});

test("OTA manifest targets the signed build 13 IPA", async () => {
  ...
  assert.match(manifest, /<key>bundle-version<\/key>\s*<string>13<\/string>/);
});
